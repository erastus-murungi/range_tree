...
if __name__ == "__main__":
    ...
    kd = KDTree(coords3)
    print(
        f"k-d tree built in {(perf_counter() - t1):.2f} seconds & used"
        f" {asizeof.asizeof(kd) / (1 << 20) :.2f} MB for {NUM_COORDS} {DIM}-D points."
    )
    t2 = perf_counter()
    vp = VPTree(coords1, minkowski)
    print(
        f"v-p tree built in {(perf_counter() - t2):.2f} seconds & used"
    ...
    print(
        f"kd kNN query ran in {(perf_counter() - t3):.2f} seconds for {k} nearest neighbors"
    )

    t4 = perf_counter()
    q2 = vp.k_nearest_neighbors(query_point, k)
    print(
        f"vp kNN query ran in {(perf_counter() - t4):.2f} seconds for {k} nearest neighbors"
    )

    t5 = perf_counter()
    q3 = brute_k_nearest_neighbors(coords2, query_point, k, minkowski)
    print(
        f"brute kNN query ran in {(perf_counter() - t5):.2f} seconds for {k} nearest neighbors"
    )
